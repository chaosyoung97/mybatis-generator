...
//            properties.load(in);
            return getPropertiesLcca("init.properties");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    private Properties getPropertiesLcca(String fileName) {
        Properties properties = new Properties();
        try {
            String outpath = System.getProperty("user.dir")+File.separator+""+File.separator;//先读取config目录的，没有再加载classpath的
            ...
            try {
                InputStream in = this.getClass().getClassLoader().getResourceAsStream(fileName);//默认加载classpath的
                properties.load(in);
...
//        genServiceImpl(targetProject, servicePackage, javaClassName, root);
    ...
    private void genService(String targetProject, String topPackage, String javaClassName, Map<String, String> root) {
        ...
        if (file.exists()) {
            System.err.println(file + " 文件已经存在.");
            return;
        } else {
            try {
                dir.mkdirs();
